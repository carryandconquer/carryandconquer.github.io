-- Insert industries that match the Key Deals sectors (using INSERT ... WHERE NOT EXISTS)
...
WHERE NOT EXISTS (SELECT 1 FROM deals_industries WHERE deals_industries.slug = v.slug);
...
WHERE NOT EXISTS (SELECT 1 FROM deals_sub_industries WHERE deals_sub_industries.slug = sub.slug);

-- Insert companies from the Key Deals data
INSERT INTO deals_companies (name, description, website, headquarters_city, state_province, country, region, external_company_id) 
SELECT name, description, website, headquarters_city, state_province, country, region, external_company_id
FROM (VALUES
  ...
  ('OmniView Sports Inc.', 'Founded in 2020 and based in Massachusetts, US, OmniView Sports Inc. operates as sports viewing application that shows its users a personalized view experience according to the user preference.', 'www.ovszone.com', 'Boston', 'Massachusetts', 'United States', 'North America', 'omniview-sports-4')
) AS v(name, description, website, headquarters_city, state_province, country, region, external_company_id)
WHERE NOT EXISTS (SELECT 1 FROM deals_companies WHERE deals_companies.external_company_id = v.external_company_id);
...
WHERE NOT EXISTS (SELECT 1 FROM deals WHERE deals.deal_id = d.deal_id);
...
WHERE NOT EXISTS (
  ...
  WHERE dci.company_id = c.id AND dci.industry_id = i.id
);

-- Insert investors
INSERT INTO deals_investors (name, type, country, region) 
SELECT name, type::investor_type, country, region
FROM (VALUES
...
) AS v(name, type, country, region)
WHERE NOT EXISTS (SELECT 1 FROM deals_investors WHERE deals_investors.name = v.name);